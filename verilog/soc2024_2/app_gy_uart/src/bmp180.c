...
int bmp180_init(void) {
    ...
    printf("BMP180 initialized successfully!\r\n");
    return 0;
}
...
int bmp180_GetResultData(BMP180_ResultData *resultData)
{
    ...
    printf("bmp180_read_raw_temperature ut : %d \n", ut);
    printf("bmp180_read_raw_pressure up : %ld \n", up);
    ...
    {// 온도 보상을 위한 값 계산
        const long x1 = ((ut - cal_data.AC6) * cal_data.AC5) >> 15;
        const long x2 = (cal_data.MC << 11) / (x1 + cal_data.MD);
        const long b5 = x1 + x2;
        const long t = (b5 + 8) >> 4;
        ...
        resultData->temperature = t / 10;
        resultData->temperatureMod = t % 10;
    }

    { // 압력 계산 (BMP180 데이터시트 알고리즘)
        // 온도 보상을 위한 값 계산 (계산 중복)            
        ...
        resultData->pressure = p / 100;
        resultData->pressureMod = p % 100;
        //printf("Pressure:    %d.%d hPa\r\n", pressure, pressureMod);

        // 압력 계산 (BMP180 데이터시트 알고리즘)
        int altitude = calc_altitude(p / 100);
        resultData->altitude = altitude;
    }
